...
- **Activatable scan must filter `IsRuntimeOnly || HiddenInUI`**: Engine-internal variant instances (e.g. ShiftersFury per-weapon facts with `ShifterFuryWeaponList` blueprint) land in `dude.ActivatableAbilities.RawFacts` alongside their parent and share the parent's `DisplayName` — without this filter the Toggle tab doubles up each variant-parent entry.
