...
- **`new AbilityData(blueprint, caster)` for items drops item caster level**: Creates an AbilityData with no `OverrideCasterLevel`/`OverrideSpellLevel`, so `CalculateParams()` uses the caster's class level. For `BlueprintItemEquipmentUsable` (scrolls/potions/wands/rods), always set both from `blueprint.CasterLevel`/`blueprint.SpellLevel` — otherwise durations and effects scale with the character, not the item.
